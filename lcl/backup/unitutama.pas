...
type
  ...
  TFormUtama = class(TForm)
    ...
    procedure BrookURLRouter1Routes5Request(ASender: TObject;
      ARoute: TBrookURLRoute; ARequest: TBrookHTTPRequest;
      AResponse: TBrookHTTPResponse);
    procedure BrookURLRouter1Routes6Request(ASender: TObject;
      ARoute: TBrookURLRoute; ARequest: TBrookHTTPRequest;
      AResponse: TBrookHTTPResponse);
...
procedure TFormUtama.BrookURLRouter1Routes6Request(ASender: TObject;
  ARoute: TBrookURLRoute; ARequest: TBrookHTTPRequest;
  AResponse: TBrookHTTPResponse);
var
  vNamaParam, vKodeParam: string;
  vJSON, vCacheKey: string;
  vRedis: TMinimalRedisClient; // Driver portable kustom Anda
  vQueryLokal: TZQuery;
  JSONArray: TJSONArray;
  JSONObject: TJSONObject;
begin
  // 1. Keamanan Pintu Depan: Validasi Token & Rate Limiting
  if not IsAuthenticatedtoken(ARequest, AResponse) then Exit;

  // Ambil parameter pencarian dari URL (?nama=... atau ?kode=...)
  vNamaParam := Trim(ARequest.Params.Values['nama']);
  vKodeParam := Trim(ARequest.Params.Values['kode']);

  // 2. Buat Key Unik untuk Redis berdasarkan Parameter Input
  // Contoh format key: cache:barang:k_laptop:n_rog
  vCacheKey := 'cache:barang:k_' + LowerCase(vKodeParam) + ':n_' + LowerCase(vNamaParam);
  vJSON := '';

  // 3. TAHAP 1: CEK LAYER MEMORI (REDIS CACHE HIT)
  vRedis := TMinimalRedisClient.Create('127.0.0.1', 6379);
  try
    try
      if vRedis.Connect then
        vJSON := vRedis.GetKey(vCacheKey); // Ambil data JSON langsung dari RAM Redis
    except
      vJSON := ''; // Fail-safe: Jika service Redis mati, API tetap jalan via DB asli
    end;

    // JIKA CACHE HIT (DATA ADA DI RAM)
    if vJSON <> '' then
    begin
      // Beri tanda pada header agar IT Manager bisa memantau performanya
      AResponse.Headers.Add('X-Cache', 'HIT - Served from Redis RAM');
      AResponse.Send(vJSON, 'application/json; charset=utf-8', 200);
      Exit; // Keluar dari fungsi! SQLite tidak perlu bekerja keras membaca SSD/Harddisk
    end;

    // 4. TAHAP 2: CACHE MISS (FALLBACK KE DATABASE SQLITE)
    if not ZConnectiondb.Connected then ZConnectiondb.Connect;
    ...
    try
      ...
      begin
        vQueryLokal.SQL.Add('AND id_barang = :kode');
        vQueryLokal.ParamByName('kode').AsString := vKodeParam;
      end;

      if vNamaParam <> '' then
      begin
        vQueryLokal.SQL.Add('AND nama_barang LIKE :nama');
        vQueryLokal.ParamByName('nama').AsString := '%' + vNamaParam + '%';
      end;

      vQueryLokal.SQL.Add('LIMIT 100');
      vQueryLokal.Open;
      ...
      // 5. TAHAP 3: TITIPKAN HASIL QUERY KE RAM REDIS UNTUK REQUEST BERIKUTNYA
      if vRedis.Connected and (vJSON <> '[]') then
      begin
        // Simpan data di RAM dengan Masa Aktif (TTL) selama 5 Menit (300 detik)
        vRedis.SetKeyEx(vCacheKey, 300, vJSON);
      end;

      // Beri tanda header bahwa data diambil langsung dari SQLite
      AResponse.Headers.Add('X-Cache', 'MISS - Fetched from SQLite DB');
      AResponse.Send(vJSON, 'application/json; charset=utf-8', 200);
    ...
    vRedis.Free; // Hancurkan objek Redis Client setelah selesai request
  end;
end;
...
begin
  ...
  FIPTracker.Sorted := False;        // Agar pencarian IP lebih cepat (Binary Search)
end;
